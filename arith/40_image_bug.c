...
void init_cvs_from_rgb_floats(void *elem, void *prev_elem);
...
void init_floats_from_rgb_ints(void *curr_elem, Pnm_rgb pix_val);
...
void init_ints_from_rgb_floats(void *elem, void *prev_elem);
...
*/
        ...
        // uncompressed = NULL; //todo comment back in after tests 
}
...
void fill_transformed_from_rgb_flt(int col, int row, A2Methods_UArray2 curr_arr, void *elem, void *trans_info)
{
        ...
        void *pix_val = info->prev_methods->at(info->prev_arr, col, row);

        fprintf(stderr, "1st MAP TRACE\n");
...
void init_cvs_from_rgb_floats(void *elem, void *prev_elem)
{
        ...
        struct rgb_float *rgb_pix = (struct rgb_float *)prev_elem;
...
A2Methods_UArray2 create_trimmed_float_arr(Pnm_ppm image, int width, int height)
{
        ...
        A2Methods_mapfun *map_p = image->methods->map_default;
        map_p(trim_arr, fill_transformed_from_rgb_ints, pixels_info);
...
{
        fprintf(stderr, "TRACING this function shouldn't be used todo delete\n");
        struct copy_info *info = trans_info;
        ...
        //void *pix_val = orig->methods->at(orig->pixels, col, row);
...
// todo: in theory could make second parameter a void ptr and dereference,
// which would let us have a single fill function (just set this to the transformation function)... todo if time, since tried but kept segfaulting/couldn't get to dereference correctly 
void init_floats_from_rgb_ints(void *curr_elem, Pnm_rgb pix_val)
{
        struct rgb_float *curr_pix = (struct rgb_float *)curr_elem;
        //Pnm_rgb pix_val = *(Pnm_rgb *)prev_elem; 

        fprintf(stderr, "TRACE rgb ints to floats (compress conversion 1) \n");
        fprintf(stderr, "int pix red val: %d \n", (pix_val)->red); // this line is seg faulting
        fprintf(stderr, "float pix red val: %.4f \n", (float)(pix_val->red));
...
void init_ints_from_rgb_floats(void *elem, void *prev_elem)
{
        fprintf(stderr, "TRACING in conversion from rgb floats to ints\n");
        Pnm_rgb int_pix = (struct Pnm_rgb *)elem;
        struct rgb_float *float_rgb = (struct rgb_float *)prev_elem;
